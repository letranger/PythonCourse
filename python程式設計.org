...
- 以[ ]將不同型態的資料含括起來，以 , 分隔
- 表列中的資料是有序排列，從0開始編號
- 格式：src_python[:exports code]{表列名稱 = [元素1, 元素2, … ]}
#+BEGIN_SRC python -r -n :results output :exports both
...
- 特列：src_python[:exports code]{tup1 = (53, )  #只有一個元素時，其後要加逗號}
*** 範例
#+BEGIN_SRC python -r -n :results output :exports both
  data = ( 'John', [95, 118], 'May', 100 )
  print(data[1])
...
- <dict>.fromkes(<seq>[, val])

| <dict>.clear()                | data.clear()                | {} |
| <dict>.copy()                 | data.copy()                 | {} |
| <dict>.get(key, default=None) | data.get(key, default=None) |    |
| <dict>.items()                | data.items()                |    |
| <dict>.keys()                 | data.keys()                 |    |
| <dict>.values()               | data.values()               |    |
| <dict>.popitem()              | data.popitem()              |    |
| <dict>.pop(key[, default])    | data.pop(key[, default])    |    |
| <dict>.fromkes(<seq>[, val])  | data.fromkes(<seq>[, val])  |    |


|data.clear()
|data.copy()
|data.get(key, default=None)
|data.items()
|data.keys()
|data.values()
|data.popitem()
|data.pop(key[, default])
|data.fromkes(<seq>[, val])
...
#+BEGIN_SRC python -r -n :results output :exports both
  ...
  print(pop1)
  print(data)
  #print("data.pop(key[, default])",data.pop(key[, default]))
  #print("data.fromkes(<seq>[, val])",data.fromkes(<seq>[, val]))
#+END_SRC
