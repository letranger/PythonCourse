...
1. Windows: 命令提示字元
    #+CAPTION: pyhone terminal mode
    #+name: fig:pythonTerminal
...
1. 結束/回到terminal
    - 下指令: src_python[:exports code]{ exit() }
    - 結束鍵: Ctrl-D
1. 此模式可配合文字編輯器撰寫程式碼，再於終端機下執行
...
1. [[https://www.spyder-ide.org/][Spyder]]
   #+CAPTION: Spyder
   #+name: fig:Spyder
   #+ATTR_LATEX: :width 400
   #+ATTR_HTML: :width 560
...
1. [[https://www.jetbrains.com/pycharm/][PyCharm]]
   #+CAPTION: PyCharm
   #+name: fig:PyCharm
   #+ATTR_LATEX: :width 400
   #+ATTR_HTML: :width 560
   ...
   [[file:PyCharm.png]]
*** Web-based環境
1. [[https://ipython.org/notebook.html][The Jupyter Notebook]]
...
1. [[https://colab.research.google.com/][Google Colaboratory]]
...
1. [[https://www.programiz.com/python-programming/online-compiler/][Python Online Compiler]]

** 兩種執行模式[fn:5]
*** Interactive mode
**** 執行: python -i filename.py
**** 優點：
...
#+end_src
**** 下載並安裝pip(命令提示字元)
- 下載pip: [[https://bootstrap.pypa.io/get-pip.py][點此下載get-pip.py]]
- 
  #+begin_src shell
  python get-pip.py
  #+end_src
...
請參閱: [[https://officeguide.cc/windows-python-jupyter-notebook-installation-tutorial-2019/][Windows 安裝 Jupyter Notebook Python 程式開發教學]]
...
#+END_SRC
- for迴圈的變數會依序走訪序列中的元素
- 序列[fn:6]可為range函式、字串(string)、表列(list)、元組(tuple)、字典(dict)、集合(set)
...
[fn:1] [[https://buzzorange.com/techorange/2019/08/20/the-history-of-python/][【Python的誕生故事】覺得其他程式語言不好用，超狂荷蘭工程師花三個月打造自己的程式語言]]
