...
- str.isdecimal(), str.isdigit(), str.isnumeric(): 字串是否為數字[fn:9]
...
** 元組 tuple
A tuple in Python is similar to a list. The difference between the two is that we cannot change the elements of a tuple once it is assigned whereas we can change the elements of a list.[fn:16]
...
*** 空集合
如果你要建立空集合，不可以直接使用{}不 包括任何元素的實字寫法，因為{}不 包括任何元素的實字寫法表示一個空字典物件，如果你要建立空集合，必須使用 set 類 別建構[fn:15]。例如：
#+begin_src python -r -n :results output :exports both
family = set()
family.add("James")
family.add("Vanessa")
...
** 作業[set]
*** 你快樂嗎？我很快樂
快樂數有以下的特性：在給定的進位制下，該數字所有數位（英語：digits）的平方和，得到的新數再次求所有數位的平方和，如此重複進行，最終結果必為 1。 [fn:13]
...
[fn:16] [[https://www.programiz.com/python-programming/tuple][Python Tuple]]

[fn:15] [[https://openhome.cc/Gossip/Python/SetType.html][集合]]

[fn:14] [[https://wenyuangg.github.io/posts/python3/python-set.html][[Python] 學習使用集合 (Set)]]

[fn:13] [[https://zh.wikipedia.org/wiki/%E5%BF%AB%E6%A8%82%E6%95%B8][快樂數]]
