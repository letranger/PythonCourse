...
- 變更預設 port: XXXX
  ...
  #+end_src
- 登出後重新登入：token
- 結束 Jupyter: 網頁右上角：Quit

#+latex:\newpage

* Hello World: C++ v.s. Python
** C++
...
**** fromkes
- <dict>.fromkes(<seq>[, val]): Python 字典 fromkeys() 函數用於創建一個新字典，以序列 seq 中元素做字典的鍵，value 為字典所有鍵對應的初始值。
#+BEGIN_SRC python -r -n :results output :exports both
  data = { 'John': 95,  'May': 100, 'John': 105 }
...
- 計算擴展歐幾里德。 輸入 a, b，輸出 a*x + b*y = gcd。如輸入 3, 5，輸出 3*2 + 5*-1 = 1
