...
\newpage
\section{Python 套件管理}
\label{sec:orga07e762}
\subsection{pip v.s. conda}
\label{sec:org3f2f04e}
\subsubsection{pip or conda}
\label{sec:org2c7d6a7}
\begin{itemize}
...
\end{itemize}
\subsubsection{conda}
\label{sec:org8ed5ee8}
\begin{itemize}
\item Conda 是一個開源的跨平台工具軟體，它被設計作為 Python、R、Lua、Scala 與 Java 等任何程式語言的套件、依賴性以及工作環境管理員，特別受到以 Python 作為主要程式語言的資料科學團隊所喜愛。
\item 傳統 Python 使用者以 pip 作為套件管理員（package manager）、以 venv 作為工作環境管理員（environment manager），而 conda 則達成了「兩個願望、一次滿足」既可以管理套件亦能夠管理工作環境。\footnote{\href{https://medium.com/datainpoint/python-essentials-conda-quickstart-1f1e9ecd1025}{輕鬆學習 Python：conda 的核心功能}\label{org4c19c05}}
\end{itemize}
\subsubsection{In both cases:}
\label{sec:orgb4c6e2e}
\begin{itemize}
\item Written in Python
\item Open source (Conda is BSD and pip is MIT)
\end{itemize}
\subsubsection{difference between conda, anaconda, and miniconda}
\label{sec:orgb56c16c}
\begin{itemize}
\item conda is both a command line tool, and a python package. \footnote{\href{https://stackoverflow.com/questions/30034840/what-are-the-differences-between-conda-and-anaconda}{What are the differences between Conda nd Anaconda?}\label{orgcf376a7}}
...
\label{sec:org04a1943}
\subsubsection{下載 v.s. 安裝}
\label{sec:orgfa2d593}
\begin{itemize}
...
\label{sec:org86f909a}
\subsubsection{爬蟲}
\label{sec:org55bb8c5}
\begin{itemize}
...
\end{itemize}
\subsubsection{網站}
\label{sec:orgf71149e}
\begin{itemize}
\item Django: 重量級網頁框架
\item Flask: 輕量級網頁框架
\end{itemize}
\subsubsection{資料處理科學計算}
\label{sec:org169147b}
\begin{itemize}
\item Numpy: 陣列矩陣神器
\item Scipy: 科學計算神器
\item Pandas
...
\label{sec:org1d45384}
\begin{itemize}
\item 建立\footnote{\href{https://ithelp.ithome.com.tw/articles/10192460}{Day01 Anaconda環境安裝!}\label{org4c94a33}}
\lstset{breaklines=true,language=shell,label= ,caption= ,captionpos=b,firstnumber=1,numbers=left}
\begin{lstlisting}
...
\section{Numpy 模組}
\label{sec:org2dd035d}
\subsection{Numpy}
\label{sec:orgc9f7a6d}
\begin{itemize}
\item Numpy 是 Python 的一個重要模組，主要用於資料處理上。Numpy 底層以 C 和 Fortran 語言實作，所以能快速操作多重維度的陣列。\footnote{\href{https://blog.techbridge.cc/2017/07/28/data-science-101-numpy-tutorial/}{從零開始學資料科學：Numpy 基礎入門}\label{org06ada5a}}
...
\label{sec:orgbb11694}
\begin{itemize}
\item Numpy 的重點在於陣列的操作，其所有功能特色都建築在同質且多重維度的 ndarray（N-dimensional array）上。
\item ndarray 的關鍵屬性是維度（ndim）、形狀（shape）和數值類型（dtype）。 一般我們稱一維陣列為 vector 而二維陣列為 matrix\textsuperscript{\ref{org06ada5a}}。
\end{itemize}
\subsubsection{一維陣列}
\label{sec:orgd42575b}
\begin{itemize}
...
\section{Matplotlib}
\label{sec:org018044b}
\subsection{Matplotlib 簡介}
\label{sec:orgf536542}
\begin{itemize}
\item NumPy 的可視化操作界面
...
\end{itemize}
\subsection{折線圖}
\label{sec:org7a6cd8a}
\subsubsection{基本語法}
\label{sec:orgac84ccb}
\begin{itemize}
...
\label{sec:orgcaca8f1}
\begin{enumerate}
\item plot()函式參數可分為兩類：fmt 字串 和 kwarg 參數
\label{sec:org852a1d4}
\begin{itemize}
...
\label{sec:org5f5ba50}
\subsubsection{語法： }
\label{sec:orgb98ca1c}
plt.pie( 比例列表 [, 參數 1, 參數 2, \ldots{}] )
\subsubsection{參數：}
\label{sec:org84c1ec0}
\begin{itemize}
...
\label{sec:orgde5ce73}
\subsubsection{語法}
\label{sec:org7e95564}
\begin{itemize}
...
\section{區域資料檔分析}
\label{sec:orge7aee58}
\subsection{Excel/CSV: Pandas 模組}
\label{sec:org9ca7392}
\subsubsection{CSV}
\label{sec:org16c6d6c}
\begin{itemize}
\item 為純文字檔，以逗號分隔值（Comma-Separated Values，CSV，有時也稱為字元分隔值，因為分隔字元也可以不是逗號）。
\item 純文字意味著該檔案是一個字元序列，不含必須像二進位制數字那樣被解讀的資料。
\item CSV 檔案由任意數目的記錄組成，記錄間以某種換行符分隔；每條記錄由欄位組成，欄位間的分隔符是其它字元或字串，最常見的是逗號或製表符。通常，所有記錄都有完全相同的欄位序列\textsuperscript{\ref{orgcf376a7}}。
\end{itemize}
\subsubsection{Pandas}
\label{sec:org21839a6}
\begin{itemize}
...
\item Pandas 主要特色有\textsuperscript{\ref{org4c94a33}}：
\begin{enumerate}
...
\label{sec:orgabc6bc5}
\subsubsection{資料選取}
\label{sec:orgdee299b}
\begin{enumerate}
...
\label{sec:org4e02265}
\begin{enumerate}
\item 語法
\label{sec:orge596beb}
\begin{itemize}
...
\label{sec:org5777d98}
\subsection{資料視覺化}
\label{sec:org03fa14d}
\subsubsection{Bar chart}
\label{sec:orgd159f2e}
\lstset{breaklines=true,language=Python,label= ,caption= ,captionpos=b,firstnumber=1,numbers=left}
\begin{lstlisting}
  # 載入函式庫
  import pandas as pd
...
\section{網路爬蟲}
\label{sec:org12b4004}
\subsection{parse JSON}
\label{sec:org2df8ecb}
\subsubsection{What is JSON?}
\label{sec:org661982b}
\begin{itemize}
...
\label{sec:orgc4f2b62}
\begin{enumerate}
\item 安裝解析 HTML 所需套件
\label{sec:orgfe7b27c}
\lstset{breaklines=true,language=shell,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
